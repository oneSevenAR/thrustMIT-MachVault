...
void setRatio(){
    ...
    FOLLOWER_STEP_NUMERATOR = (long)(STEPS_PER_MM * wire_pitch_mm * FOLLOWER_MICROSTEP_FACTOR*2);
...
void follower_step(int change = 0) {
...
void loop() {
  ...
  switch (program_state) {
    ...
    case 3: { // Set Coil Length
      ...
      if (ent_fell) {
        follower_max_steps = 4500;//(long)coil_length_mm * STEPS_PER_MM;
        program_state++;
        lcd.clear();
    ...
    case 5: { // Jogging and Manual Home Position Setup (PRE-WINDING)
      ...
      if (up_fell) {
        ...
        if (wind_direction){
          wind_direction =0;
            digitalWrite(follower_dir_pin, HIGH);
            lcd.setCursor(0, 0); lcd.print("Going Right");
        }
        else {
            wind_direction = 1;
          digitalWrite(follower_dir_pin, LOW);
          lcd.setCursor(0, 0); lcd.print("Going Left");
        }
    ...
    case 6: { // Winding in Progress
        ...
        if (accelerating && ROTOR_STEP_DELAY_US > ROTOR_TARGET_DELAY) {
            ROTOR_STEP_DELAY_US -= accel_step;  // Accelerate
        } else if (!accelerating && ROTOR_STEP_DELAY_US < 2000) {
            ROTOR_STEP_DELAY_US += decel_step;  // Decelerate
        }
        ...
        if (ent_fell) {
                      // Stop rotor pulses
                      accelerating = false;  // Start deceleration
                    rotorStepState = LOW;
                    digitalWrite(rotor_step_pin, LOW);

                      // Toggle winding direction
                    if (wind_direction == 0) {
                        wind_direction = 1;
                        digitalWrite(follower_dir_pin, LOW);   // Reverse
                        Serial.println("Direction -> REVERSE");
                    } else {
                        wind_direction = 0;
                        digitalWrite(follower_dir_pin, HIGH);  // Forward
                        Serial.println("Direction -> FORWARD");
                    }

                      accumulator = 0; // reset fractional tracking
                      delay(300); // pause before restarting to stabilize
                      ROTOR_STEP_DELAY_US = 2000;   // Reset to slow start for next direction
                      accelerating = true;     // Prepare to ramp up again
                      program_state = 4;
