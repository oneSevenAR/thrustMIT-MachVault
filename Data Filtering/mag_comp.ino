...
public:
    ...
    void update(Vector3 gyro_rad_s, Vector3 accel_g, float dt) {
        ...
        J[1][0] = 2*q_norm.x;  J[1][1] = 2*q_norm.w; J[1][2] = 2*q_norm.z; J[1][3] = 2*q_norm.y;
        ...
        if (grad_mag != 0.0f) { // Normalize gradient
            gradient[0] /= grad_mag; gradient[1] /= grad_mag;
...
void setup() {
    ...
    for (int i = 0; i < calibration_samples; i++) {
        ...
        temp_gyro_sum.x += g.x; temp_gyro_sum.y += g.y; temp_gyro_sum.z += g.z;
...
void loop() {
    ...
    // Inside your loop() function, after reading raw data:
Serial.print("Raw Accel (X,Y,Z): ");
Serial.print(accel_raw.x, 4); Serial.print(", ");
Serial.print(accel_raw.y, 4); Serial.print(", ");
Serial.println(accel_raw.z, 4);

Serial.print("Raw Gyro (X,Y,Z): ");
Serial.print(gyro_raw.x, 4); Serial.print(", ");
Serial.print(gyro_raw.y, 4); Serial.print(", ");
Serial.println(gyro_raw.z, 4);
// Add a small delay here to make raw data readable on serial monitor
delay(100);
    ...
    calibrated_accel.z = accel_raw.z - (accel_baseline.z - 1.0f); // Adjust Z baseline to be relative to 1g
    ...
    // 1. Always update the Complementary Filter (fast, low-lag output)
    complementaryFilter.update(calibrated_gyro, calibrated_accel, dt);
    float comp_pitch_deg = complementaryFilter.getPitchDegrees();
    float comp_roll_deg = complementaryFilter.getRollDegrees();

    // 2. Periodically update the Madgwick Filter and use its output for correction
    bool correction_applied = false;
    iteration_count++;
    if (iteration_count >= MADGWICK_UPDATE_INTERVAL_ITERATIONS) {
        madgwickFilter.update(calibrated_gyro, calibrated_accel, dt); // Update Madgwick
        Vector3 madg_euler = quaternionToEulerAngles(madgwickFilter.q);

        // Correct the Complementary Filter's state with Madgwick's more accurate estimate
        complementaryFilter.pitch = madg_euler.y * M_PI / 180.0f; // Convert back to radians
        complementaryFilter.roll = madg_euler.x * M_PI / 180.0f;
        correction_applied = true;
        iteration_count = 0; // Reset counter

        Serial.print("Madgwick corrected Complementary Filter. Madgwick: P=");
        Serial.print(madg_euler.y, 2);
        Serial.print(" R=");
        Serial.println(madg_euler.x, 2);
    }
    
    // Get Madgwick's current Euler angles for logging (even if not updated this iteration)
    Vector3 madg_current_euler = quaternionToEulerAngles(madgwickFilter.q);

    // Log data from both filters to Serial Plotter/Monitor
    Serial.print(millis()); Serial.print(",");
    Serial.print(comp_pitch_deg, 4); Serial.print(",");
    Serial.print(comp_roll_deg, 4); Serial.print(",");
    Serial.print(madg_current_euler.x, 4); Serial.print(","); // Madgwick Roll
    Serial.print(madg_current_euler.y, 4); Serial.print(","); // Madgwick Pitch
    Serial.print(madg_current_euler.z, 4); Serial.print(","); // Madgwick Yaw
    Serial.println(correction_applied ? "Yes" : "No");

    // Optional: Add a small delay if the loop runs too fast for your serial monitor
    // or if you need to limit the data rate.
    // However, for minimal lag, aim for highest possible rate.
    // delay(1); // Example: 1ms delay
}
